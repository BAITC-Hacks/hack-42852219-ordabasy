...
export function SimulationTimeline() {
  return (
    <section
      aria-live="polite"
      className="mx-auto w-full max-w-4xl rounded-3xl border border-teal-200 bg-white p-8 text-center shadow-xl"
    >
      ...
      <h2 className="mt-5 text-2xl font-black text-slate-950">
        Симулируем следующие 8 кварталов
      </h2>
      ...
      <ol className="mt-8 flex items-center">
        {Array.from({ length: SIMULATION_QUARTERS }, (_, index) => (
          <li key={index} className="flex flex-1 items-center last:flex-none">
            <div
              className="grid size-9 shrink-0 animate-[quarterActivate_260ms_ease-out_forwards] place-items-center rounded-full bg-slate-200 text-xs font-black text-slate-500"
            ...
            {index < SIMULATION_QUARTERS - 1 && (
              <div
                className="h-1 flex-1 origin-left animate-[timeline_180ms_ease-out_forwards] bg-teal-200"
